...
function ProtectedRouteWrapper() {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        const response = await axios.get(`${API}/middleware`, { withCredentials: true });
        ...
        console.error('Auth check failed:', err.message, err.response?.status);
        if (err.response?.status === 404) toast.error('Authentication service unavailable.');
        else if (err.response?.status === 401) toast.error('Unauthorized, log in.');
        else toast.error('Error verifying authentication.');
        setIsAuthenticated(false);
        navigate("/login");
      }
    };

    if (!window.location.pathname.startsWith("/admin")) checkAuth();
